...
**Scope.** ERC-8004 Reputation Registry on Celo mainnet, blocks 58396729–76120647 (2026-02-08 → 2026-08-25).
...
| …claiming a specific payment transaction | 15 (0.1%) |
| …whose claimed transaction exists on chain | 2 (0.0%) |
| …whose payment actually verifies | **2 (0.0%)** |
...
- Distinct agents rated: **7,743** of 9,788 registered
...
| …of which the file actually resolved | 619 | 2.2% |
| Declares a non-zero `feedbackHash` | 16,533 | 60.1% |
| Hash attested but no file published | 9,628 | 35.0% |
| File retrievable | 619 | 2.2% |
| File matches the attested hash | 228 | 0.8% |
| Contains a payment claim | 15 | 0.1% |
| Claimed transaction exists on chain | 2 | 0.0% |
| Payment verified — exists, succeeded, moved value | **2** | **0.0%** |
| Payment attributed — …and paid by this reviewer to this agent | **0** | **0.0%** |
| Payment cited but its parties contradict the claim | 2 | 0.0% |
| Payment declared on a chain this audit does not query | 0 | 0.0% |

> **Not a finding:** 543 records could not be retrieved for
...
> 619 retrieved files were archived content-addressed under
> `out/evidence-corpus/`, so every verdict above stays checkable after the
> originals go offline.

> Sampled 1,745 of 10,469 records that declare a file — every 6th, evenly spread across the period rather than taken from one end. The four rows below are counts within that sample; the percentages are of all feedback, so read them as a lower bound.
